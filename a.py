...
class Score:
    ...
    def __init__(self):
        ...
        self.score = 0
        self.text = self.font.render(f"Score: {self.score}", True, (0, 0, 255))
    ...
    while True:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                beams.append(Beam(bird)) # 制限なしでビームを追加

        screen.blit(bg_img, [0, 0])
        
        # 爆弾リストからNoneを除外（消滅した爆弾の掃除）
        bombs = [bomb for bomb in bombs if bomb is not None]
        
        # ビームリストから画面外に出たビームを除外（消滅したビームの掃除）
        ...
        for i, bomb in enumerate(bombs):
            if bomb is not None and bird.rct.colliderect(bomb.rct):
                ...
                score.update(screen) # スコアの最終表示
                pg.display.update()
                time.sleep(3) # 3秒間表示して終了
                return
        ...
        for bomb_idx in range(len(bombs) - 1, -1, -1):
            ...
            for beam_idx in range(len(beams) - 1, -1, -1):
                ...
                if beam.rct.colliderect(bomb.rct):
                    ...
                    # 練習問題3: 爆弾撃墜でこうかとんをハッピー状態にする
                    bird.set_happy() 
                    break # このビームは消えたので、次の爆弾のチェックへ

        # 演習問題2: 複数ビームの更新・描画
        for beam in beams:
            beam.update(screen)
                
        # 練習問題5: 爆弾リストの全要素を更新・描画
        for bomb in bombs:
            bomb.update(screen)
